...
namespace BatchDotnetTutorialFfmpeg
{
    ...
    public class Program
    {
        ...
        private const string BatchAccountName = "batchaccountmaybe";
        private const string BatchAccountKey = "+uzWsB8Ava/svRN6ADdBhSKp4ElqG0OOBkFGcga1E7QvxOSjclPQjIO4FHqF47WCgZhJ+LHhe3uAjCuZRHp/Hw==";
        private const string BatchAccountUrl = "https://batchaccountmaybe.westeurope.batch.azure.com";

        // Storage account credentials
        private const string StorageAccountName = "generalstorage02192";
        private const string StorageAccountKey = "bF1eOCNSOea/RLH1T86pjyqapxcoY63e1XOdTKw97fz0KW7sz6xRQELJ+T7N2RvWNWO5FaQCW1zLQE+dA/edvg==";

        // Pool and Job constants
        private const string PoolId = "WinFFmpegPool";
        private const int DedicatedNodeCount = 5;
        private const int LowPriorityNodeCount = 0;
        ...
        const string appPackageId = "SimpleNavier";
        const string appPackageVersion = "0.1";
        ...
        private static async Task MainAsync()
        {
            ...
            List<string> inputFilePaths = new List<string>(Directory.GetFileSystemEntries(inputPath, "*.txt",
        ...
        private static async Task<List<CloudTask>> AddTasksAsync(BatchClient batchClient, string jobId, List<ResourceFile> inputFiles, string outputContainerSasUrl)
        {
            ...
            for (int i = 0; i < inputFiles.Count; i++)
            {
                ...
                string prefix = "ResultOf";
                string outputMediaFile = String.Format("{0}{1}{2}", prefix,
                    System.IO.Path.GetFileNameWithoutExtension(inputMediaFile),
                    ".txt");

                // String that is required for this stuff to work
                string numberOfIteration = Convert.ToString(i);

                //string taskCommandLine = String.Format("cmd /c {0}\\ffmpeg-3.4-win64-static\\bin\\ffmpeg.exe -i {1} {2}", appPath, inputMediaFile, outputMediaFile);
                //string taskCommandLine = String.Format("cmd /c {0}\\LetsDoIt\\spoko.exe {1} {2}", appPath, outputMediaFile, numberOfIteration);
                string taskCommandLine = String.Format("cmd /c {0}\\LetsDoIt\\spoko.exe {1} {2} {3}", appPath, inputMediaFile, outputMediaFile, numberOfIteration);
